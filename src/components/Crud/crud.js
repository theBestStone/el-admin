  ...
  const methods = {
    ...
    refresh() {
      ...
      return new Promise((resolve, reject) => {
        crud.loading = true
        let getQueryParams =  crud.getQueryParams()
        // 请求数据
        if(!getQueryParams.createTime){
          getQueryParams.createTime = [];
        }
        // console.log(crud.getQueryParams())
        initData(crud.url, getQueryParams)
          .then(data => {
            ...
            if(data.data.cpuUsage){//单独处理资源管理
              let arr = [
                {cpuUsage:data.data.cpuUsage,diskUsage:data.data.diskUsage,memoryUsage:data.data.memoryUsage}
              ]
              crud.page.total = 1
              crud.data = arr;
            }else{
     ...
     */
    toEdit(data,views) {
      let copyFormData = JSON.parse(JSON.stringify(data))
      for (const key in copyFormData) {
        if (Object.hasOwnProperty.call(copyFormData, key)) {
          if (typeof copyFormData[key] == 'number') {
            copyFormData[key] = copyFormData[key].toString()
          }
        }
      }
      crud.resetForm(copyFormData)
      if (!(callVmHook(crud, CRUD.HOOK.beforeToEdit, crud.form) && callVmHook(crud, CRUD.HOOK.beforeToCU, crud.form))) {
        return
      }
      crud.status.edit = CRUD.STATUS.PREPARED;
      if(views){
        crud.status.view = true;
      }else{
        crud.status.view = false;
      }
      console.log(crud.status.view)
    ...
    doExport() {
      ...
      let str =""
      let formData = crud.getQueryParams();
      ...
      let url =process.env.VUE_APP_BASE_API+"/api-manage/"+ crud.url + '/download'+ '?access_token=' + getToken()+"&t="+Math.random()+str;
    ...
    getQueryParams: function() {
      ...
      Object.keys(crud.query).length !== 0 &&
        Object.keys(crud.query).forEach(item => {
          ...
          if (crud.query[item] === null || crud.query[item] === '') crud.query[item] = ''
        })
    ...
    dleChangePage(size) {
      if (crud.data.length === size && crud.page.pageNum !== 1) {
        crud.page.pageNum -= 1
        crud.page.pageNum = crud.page.pageNum + ''
      }
    },
    // 选择改变
