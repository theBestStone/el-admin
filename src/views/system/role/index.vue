  ...
  <div class="app-container">
    ...
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible="crud.status.cu > 0" :title="crud.status.title" width="520px">
      <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="80px">
        ...
        <el-form-item label="角色描述" prop="roleDesc" verify>
          <el-input v-model="form.roleDesc" style="width: 380px;" rows="5" type="textarea" />
        </el-form-item>
        <el-form-item label="角色状态" prop="state" verify>
        ...
        <el-card class="box-card" shadow="never">
          ...
          <el-table ref="table" v-loading="crud.loading" highlight-current-row style="width: 100%;" :data="crud.data" @selection-change="crud.selectionChangeHandler" @current-change="handleCurrentChange">
            ...
            <el-table-column prop="state" label="角色状态">
              <template slot-scope="scope"> {{ scope.row.state == 0 ? '开启' : '关闭' }} </template>
            </el-table-column>
            <el-table-column :show-overflow-tooltip="true" prop="roleDesc" label="描述" />
            <el-table-column :show-overflow-tooltip="true" width="135px" prop="createTime" :formatter="$util.YYMM" label="创建日期" />
...
const defaultForm = { menuIdlist: [], roleCode: '', roleDesc: '', roleName: '', state: '0' }
export default {
  name: 'Role',
  ...
  data() {
    return {
      defaultProps: { children: 'children', label: 'menuName', isLeaf: 'hasChildren' },
  ...
  created() {
    // console.log(common.base64DeCode('ac3lzLXJvbGUvZmluZFJvbGVMaXN0'),"2222222222222")
  ...
  methods: {
    getMenuDatas(node, resolve) {
      setTimeout(() => {
        getMenusTree({ parentId: node.data.id ? node.data.id + '' : '0' }).then(res => {
          console.log(res)
          resolve(res.data)
        })
      }, 100)
    ...
    handleCurrentChange(val) {
      if (val) {
        ...
        // 初始化默认选中的key
        this.menuIds = []
        val.menuList.forEach(function(data) {
    ...
    menuChange(menu) {
      // 获取该节点的所有子节点，id 包含自身
      getChild({ parentId: menu.id + '' }).then(({ data }) => {
        // 判断是否在 menuIds 中，如果存在则删除，否则添加
        if (this.menuIds.indexOf(menu.id) !== -1) {
          for (let i = 0; i < data.length; i++) {
            const index = this.menuIds.indexOf(data[i])
            if (index !== -1) {
    ...
    update() {
      // 无刷新更新 表格数据
      crudRoles.get({ roleId: this.currentId + '' }).then(res => {
        // console.log(res)
        // console.log(this.crud.data);
        this.crud.data.find(item => item.id == this.currentId).menuList = res.data
