<template>
  <div class="app-container">
    ...
    <!--表格渲染-->
    <el-table ref="table"  lazy :load="getDeptDatas" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }" :data="crud.data" row-key="id" @select="crud.selectChange" @select-all="crud.selectAllChange" @selection-change="crud.selectionChangeHandler">
      <!-- <el-table-column :selectable="checkboxT" type="selection" width="55" /> -->
      <el-table-column label="CPU百分比" prop="cpuUsage" />
      <el-table-column label="内存使用率" prop="memoryUsage" />
      <el-table-column label="磁盘使用率" prop="diskUsage" />
      <el-table-column label="网络运行状态" prop="networkStatus">
        <template slot-scope="scope">
          正常
          <!-- {{ scope.row.networkStatus == 0 ? '正常' : '不稳定' }} -->
        </template>
      </el-table-column>
      <el-table-column label="数据库运行状态" prop="databaseStatus">
        <template slot-scope="scope">
          正常
          <!-- {{ scope.row.databaseStatus == 0 ? '正常' : '不稳定' }} -->
        </template>
      </el-table-column>
      <!-- <el-table-column v-if="checkPer(['admin', 'user:edit', 'user:del'])" label="操作" width="115" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button type="primary" v-if="scope">查看详情</el-button>
        </template>
      </el-table-column> -->
    </el-table>
  </div>
...
export default {
  name: 'Dept',
  components: { Treeselect, rrOperation, udOperation, DateRangePicker },
  cruds() {
    return CRUD({
      title: '部门',
      url: common.base64DeCode('93E9ADE1D130CA36D0082296280A9AF8'),//'/server/info',
      crudMethod: { ...crudDept },
      optShow: {
        add: false,
        edit: false,
