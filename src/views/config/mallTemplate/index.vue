  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        ...
        <el-select v-model="query.useType" placeholder="请选择" @change="crud.toQuery" style="float: left;margin-right: 10px;top: -1px;">
          <el-option
    ...
    <!--表格渲染-->
    <el-table ref="table" v-loading="crud.loading" lazy :data="crud.data" row-key="id" @select="crud.selectChange" @select-all="crud.selectAllChange" @selection-change="crud.selectionChangeHandler">
      <el-table-column type="selection" width="55" />
      <el-table-column label="模版名称" prop="templateName" />
      <el-table-column label="模版用途">
        <template slot-scope="scope">
          <span>{{ scope.row.useType === 1? '场景模版': (scope.row.useType === 1 ?'组建模版' : '') }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  cruds() {
    return CRUD({
      title: '场景业务类型',
      url: common.base64DeCode('536B11C98C313FFA501F86A6D089EC8EDF2FDEE5FE757107E959BA38321F5B8F40457E0A85844ABC27D4046A22BEC699'),//'/shopTemplate/findShopTemplateList',
      crudMethod: { ...crudDow },
      optShow: {
        add: true,
        edit: false,
