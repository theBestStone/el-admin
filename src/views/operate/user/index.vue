  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        ...
        <el-input v-model="query.account" clearable size="small" placeholder="输入用户账号搜索" style="width: 200px;" class="filter-item" @keyup.enter.native="crud.toQuery" />
    ...
    <el-table ref="table" v-loading="crud.loading" lazy :load="getDeptDatas" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }" :data="crud.data" row-key="id" @select="crud.selectChange" @select-all="crud.selectAllChange" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column label="注册时间" prop="createTime" :formatter="$util.YYMM" />
      <el-table-column label="用户状态" prop="realnameState">
        <template slot-scope="scope">
          {{ scope.row.realnameState == 0 ? '未实名' : '已实名' }}
        </template>
      </el-table-column>
      <el-table-column label="所属企业名称" prop="orgNameAll" />
    </el-table>
    <!--分页组件-->
    <pagination />
  </div>
...
export default {
  ...
  watch: {
    'crud.page.total'(val) {
      this.$emit('callbackNum', { type: 'newVisitis', num: val })
    }
  },
  data() {
    return {
