  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        ...
        <el-select v-model="query.delFlag" clearable size="small" placeholder="企业状态" class="filter-item" @change="crud.toQuery">
    ...
    <el-table ref="table" v-loading="crud.loading" lazy :load="getDeptDatas" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }" :data="crud.data" row-key="id" @select="crud.selectChange" @select-all="crud.selectAllChange" @selection-change="crud.selectionChangeHandler">
      <el-table-column label="企业编码" prop="eid" />
      <el-table-column label="企业名称" prop="orgName" />
      <el-table-column label="企业邀请人" prop="sysUserName" />
      <el-table-column label="邀请人账号" prop="sysAccount" />
      <el-table-column label="企业状态" prop="delFlag">
        <template slot-scope="scope">
          {{ scope.row.delFlag == 0 ? '正常' : '注销' }}
        </template>
      </el-table-column>
      <el-table-column label="企业创建时间" prop="createTime" :formatter="$util.YYMM" />
    </el-table>
    <!--分页组件-->
    <pagination />
  </div>
...
export default {
  ...
  watch: {
    'crud.page.total'(val) {
      this.$emit('callbackNum', { type: 'purchases', num: val })
    }
  },
  data() {
    return {
